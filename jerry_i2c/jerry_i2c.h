...
bool I2C_send_long_byte(I2C_Pin_Content *pin, const uint8_t* data, const size_t length){
	for(size_t i = 0; i < length ; ++i){
		if(!(I2C_send_one_byte(pin, data[i]))){
			return false;
		}
	}
	return true;
...
uint8_t I2C_read_once_byte(I2C_Pin_Content *PIN_CONTENT){
	...
	for(int i = 0; i < 8; ++i){
		...
		I2C_LOOP_nop(I2C_loop_nop_number);
		temp_byte <<= 1;
		temp_byte = temp_byte | (HAL_GPIO_ReadPin(PIN_CONTENT->GPIOX, PIN_CONTENT->SDA) == GPIO_PIN_SET ? 1 : 0);
